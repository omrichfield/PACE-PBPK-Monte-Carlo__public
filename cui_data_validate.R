...
organ_FACS <- data.frame(read_excel("data_for_model/cui_validate_mfi.xlsx"))
organ_vol_vec <- c(VLC, VLuC, VSC, VHC, VKC)
organ_FACS_cui <- data.frame(mn = 0*organ_vol_vec, sd = 0*organ_vol_vec)

Fluor_vol <- as.numeric(t(organ_FACS[4,2:dim(organ_FACS)[2]]*organ_vol_vec))
Fluor_vol_sd <- as.numeric(t(organ_FACS[6,2:dim(organ_FACS)[2]]*organ_vol_vec))
organ_FACS_cui$mn <- Fluor_vol
organ_FACS_cui$sd <- Fluor_vol_sd
organ_FACS_cui$Liver_norm_mn <- organ_FACS_cui$mn/organ_FACS_cui$mn[1]
organ_FACS_cui$organs <- c('Liver','Lungs','Spleen','Heart','Kidneys')

writeMat('model_generated_data/organ_FACS_cui_20231210.mat',organ_FACS_cui=organ_FACS_cui)

# ##############################################################################################
# #Organ NP FACS in time
# 
# organ_FACS <- data.frame(read_excel("organ_FACS_MFI_time.xlsx"))
# 
# organ_vol_vec <- c(rep(V_Heart,n),
#                    rep(V_Lung,n),
#                    rep(V_Liver,n),
#                    rep(V_Spleen,n),
#                    rep(V_Kidney,n),
...
blood_conc_temp <- data.frame(read_excel("data_for_model/cui_validate_time.xlsx"))

blood_conc_cui <- data.frame(time=blood_conc_temp$time, 
                             mean=blood_conc_temp$mean_tr,
                             sd=blood_conc_temp$SD_tr)

writeMat('model_generated_data/cui_blood_meas_20231210.mat',blood_conc_cui=blood_conc_cui)
